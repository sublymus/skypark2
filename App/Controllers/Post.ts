...
import { NotificationController } from "../Models/NotificationModel";
type DOC<Ctrl extends ModelControllerInterface<any, any, any, any>> =
  Ctrl extends ModelController<any, any, infer DES>
    ? mongoose.Document<any, any, superD<DES>> &
        superD<DES> & { [k: string]: any }
    : ModelController<any, any, any>;
  ...
  services: {
    ...
    survey: async (ctx) => {
      try {
        ...
        const survey = await SurveyController.model.findOne({
          _id: post.survey?.toString(),
        });

        if (!survey?.options) return;
        if ((survey?.__createdAt || 0) + (survey?.delay || 0) < Date.now())
          return;

        Log("survey", survey);
        const labelsPromise = survey.options.map(async (vote) => {
          return (
            LabelController.model.findOne({ _id: vote.toString() }) || null
          );
        });
        ...
        let limite =
          (survey?.__createdAt || 0) + (survey?.delay || 0) - Date.now();
    ...
    statPost: async (ctx: ContextSchema): ResponseSchema => {
      try {
        ...
        const accountPost = await AccountController.model.findOne({
          __key: post.__key,
        });

        if (!accountPost)
          return {
            error: "accountPost est null",
            status: 404,
            code: "ERROR|Post:account|NOT_FOUND",
            message: "le post n'a pas de account",
          };
        const notification = await NotificationController.model.findOne({
          _id: accountPost?.notification?.toString(),
        });
        ...
        /*************************     SHARED      ********************** */
        if (accountShared) {
          const accountSharedInsatance = await AccountController.model.findOne({
            _id: accountShared,
          });
        ...
        if (historique && change) {
          ...
          try {
            ...
            console.log("🚀 ~ statPost: ~ error:", error);
          }
        }
        /*************************    FAVORITES    ***********************/
        const favorites = await ModelControllers["favorites"].model.findOne({
          _id: account?.favorites?.toString(),
        });
        if (favorites) {
          if (favorite == true) {
            //@ts-ignore
        ...
        if (ctx?.login.id && newPostData) {
          const res = await PostModelCTRL.services["list"]?.({
            ...
            data: {
              ...
              paging: {
                query: {
                  __parentModel:
                    "post" + "_" + postId + "_" + "comments" + "_post",
                },
              },
            },
          });
          ...
          newComment = await PostModelCTRL.model.findOne({
            _id: res.response.added[0],
          }); //////
          if (newComment) {
            change = true;
            // {triggerAccountId:string,targetPostId:string,mode:string,value : string ,checked:boolean}

            notif.push({
              triggerAccountId: ctx.login.id,
              targetPostId: postId,
              value: newComment._id.toString(),
              mode: "comment",
              checked: false,
            });
          }
        }
        if (notification && notif.length > 0) {
          //@ts-ignore
          notification.elements = [...notif, ...notification.elements!];
          notification.save();
        }
